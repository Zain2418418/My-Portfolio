...
export default function Stats() {
  ...
  return (
    // Fluid horizontal padding to support narrow mobile screens
    <section className="w-full bg-[#14342b] text-[#fbf9f4] py-8 md:py-10 px-4 sm:px-8 md:px-16 border-t border-[#fbf9f4]/10">
      ...
      <div className="mx-auto max-w-5xl grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 sm:gap-x-6 text-center items-center">
        {statsData.map((stat, index) => (
          <div key={index} className="flex flex-col items-center space-y-1 group w-full">
            ...
            <span className="text-[11px] sm:text-xs md:text-sm text-stone-300/80 font-sans tracking-wide max-w-[130px] sm:max-w-[160px] leading-snug break-words">
              {stat.label}
            </span>
